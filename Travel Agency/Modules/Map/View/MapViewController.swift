...
class MapViewController: UIViewController {
    ...
    private func focusOnPlace(_ place: MapModel) {
        ...
        if let annotation = mapView.annotations.first(where: {
            $0.coordinate.latitude == lat && $0.coordinate.longitude == lng
        }) {
    ...
    private func showAllPlaces() {
        ...
        let annotations: [CustomAnnotation] = viewModel.places.compactMap { place in
            guard let lat = place.lat, let lng = place.lng else { return nil }
            let imgPath = place.img ?? ""
            let fullUrl = baseUrl + (imgPath.hasPrefix("/") ? "" : "/") + imgPath
            return CustomAnnotation(coordinate: CLLocationCoordinate2D(latitude: lat, longitude: lng),
                                    title: place.name,
                                    imageUrl: fullUrl)
        }
...
extension MapViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        ...
        if let index = viewModel.places.firstIndex(where: {
            $0.lat == annotation.coordinate.latitude &&
            $0.lng == annotation.coordinate.longitude
        }) {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "MapLocationCell", for: indexPath) as! MapLocationCell
